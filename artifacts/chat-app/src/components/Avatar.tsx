...
function getColor(name: string) {
  const colors = [
    "bg-[#128C7E]", "bg-[#075E54]", "bg-purple-500", "bg-blue-500",
  ...
  return (
    <div className={`relative flex-shrink-0 ${className}`}>
      {src ? (
        ...
        <div className={`${sizeMap[size]} ${getColor(name)} rounded-full flex items-center justify-center text-white font-semibold`}>
          {getInitials(name)}
        </div>
      )}
      {online !== undefined && (
